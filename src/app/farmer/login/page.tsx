...
import { CROP_NAMES, DISTRICT_NAMES } from "@/lib/i18n/ui";
    ...
    <div className="min-h-dvh bg-sand-50 sm:bg-sand-200 sm:py-6">
      <div className="relative mx-auto flex min-h-dvh sm:min-h-[calc(100dvh-3rem)] w-full max-w-[440px] flex-col overflow-hidden bg-sand-50 sm:rounded-[2.2rem] sm:border sm:border-ink-200/70 sm:shadow-lift">
        <div className="relative gradient-forest px-6 pt-12 pb-16 text-white overflow-hidden">
          ...
          <div className="relative mt-4 inline-flex items-center gap-1.5 rounded-full bg-white/10 border border-white/15 px-3 py-1 text-[11px] font-semibold"><ShieldCheck className="h-3.5 w-3.5" /> {t.govUniqueId}</div>
        </div>
        ...
        <div className="relative -mt-8 flex-1 px-5 pb-8">
          <div className="card-surface p-5">
            <div className="mb-4 inline-flex w-full rounded-2xl bg-ink-100 p-1">
              {(["login", "register"] as const).map((m) => (
                <button key={m} onClick={() => { setMode(m); setStep("id"); }} className={cn("flex-1 h-9 rounded-xl text-sm font-semibold transition-colors", mode === m ? "bg-white shadow-soft text-forest-900" : "text-ink-500")}>{m === "login" ? t.login : t.register}</button>
              ))}
            </div>
            ...
            <AnimatePresence mode="wait">
              {step === "id" ? (
                <motion.div key={mode} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} className="space-y-3">
                  {mode === "register" && (
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <label className="block text-xs font-semibold text-ink-600">{t.district}<Select className="mt-1" defaultValue="nashik">{DISTRICTS.map((d) => <option key={d.id} value={d.id}>{DISTRICT_NAMES[lang][d.id]}</option>)}</Select></label>
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <label className="block text-xs font-semibold text-ink-600">{t.currentCrop}<Select className="mt-1" defaultValue="tomato">{CROPS.map((c) => <option key={c.id} value={c.id}>{c.emoji} {CROP_NAMES[lang][c.id]}</option>)}</Select></label>
                      </div>
                    </>
                  )}
                  ...
                  <button onClick={() => { setId(DEMO_FARMER.id); }} className="w-full text-center text-xs text-forest-700 font-semibold hover:underline">{t.useDemoId} · {DEMO_FARMER.id}</button>
                </motion.div>
              ) : (
          ...
          <div className="mt-5 grid grid-cols-3 gap-2 text-center text-[11px] text-ink-500">
            {[{ i: Sprout, l: t.cropDiagnosis }, { i: MapPin, l: t.localAlerts }, { i: ShieldCheck, l: t.expertVerified }].map((x) => {
              const Icon = x.i;
