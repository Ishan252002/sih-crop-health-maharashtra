...
import { CROPS, DISTRICTS, THREATS, cropById, districtById } from "@/lib/mock/geo";
import { weatherFor } from "@/lib/mock/weather";
import { computeRisk } from "@/lib/risk-engine";
import { HOTSPOTS } from "@/lib/mock/hotspots";
import { CROP_NAMES, STAGE_NAMES, THREAT_NAMES } from "@/lib/i18n/ui";
    ...
    <FarmerShell title={t.riskForecast} back="/farmer">
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-2">
          ...
          <Select value={districtId} onChange={(e) => setDistrictId(e.target.value)}>{DISTRICTS.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</Select>
        ...
        <motion.div key={`${risk.level}-${risk.score}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card-surface p-4">
          ...
          <div className="mt-2 flex justify-center"><RiskGauge score={risk.score} level={risk.level} size={240} levelLabel={riskLabel} sublabel={`${districtById(districtId).name} · next 72 h`} /></div>
        ...
        <WeatherStrip weather={weather} districtName={districtById(districtId).name} />

        <div className="card-surface p-4">
          <div className="font-display font-bold text-ink-900">Risk factors</div>
            ...
            {risk.factors.map((f, i) => (
              <motion.div key={f.label} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between text-xs"><span className="font-semibold text-ink-800">{f.label}</span><span className="text-ink-500">{f.value}</span></div>
