    ...
    <div className="space-y-3">
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="card-surface overflow-hidden">
        <div className="relative">
          ...
          <img src={image} alt="Scanned crop" className="aspect-[16/10] w-full object-cover" />
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="rounded-2xl border border-amber-500/40 bg-amber-100/60 p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="mt-0.5 text-xs font-semibold text-amber-600">AI Confidence: {ai.confidence}% → Expert Review Required</div>
              <p className="mt-1.5 text-[13px] text-ink-700">{t.expertReviewBody}</p>
            </div>
          </div>
        </motion.div>
      ) : (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex items-center gap-2 rounded-2xl border border-forest-200 bg-forest-50 px-4 py-3 text-sm text-forest-800"><CheckCircle2 className="h-4.5 w-4.5" /> High confidence. Advisory is ready. Case is auto-logged for district surveillance.</motion.div>
      )}
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card-surface p-4">
        ...
        <ul className="mt-2 grid gap-1.5">
          {ai.symptoms.map((s) => (
            <li key={s} className="flex items-start gap-2 text-[13.5px] text-ink-700"><span className="mt-[7px] h-1.5 w-1.5 shrink-0 rounded-full bg-amber-500" />{s}</li>
          ))}
        </ul>
        {!compact && (
          <details className="mt-3 group">
            ...
            <ol className="mt-2 space-y-1.5 text-[13px] text-ink-600">
              {ai.reasoning.map((r, i) => (
                <li key={i} className="flex gap-2"><span className="font-mono text-ink-400">{i + 1}.</span>{r}</li>
              ))}
            </ol>
            {ai.alternatives.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-1.5 text-[11px]">
                <span className="text-ink-500">Alternatives:</span>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="card-surface p-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[11px] text-ink-500">weather + soil + stage</span>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="card-surface p-4">
        ...
        <ol className="mt-2 space-y-2 text-[13.5px] text-ink-700">
          <li className="flex gap-2"><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-forest-800 text-[11px] font-bold text-white">1</span>Remove and destroy affected lower leaves today. Do not compost.</li>
          <li className="flex gap-2"><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-forest-800 text-[11px] font-bold text-white">2</span>Switch to base irrigation in the morning; avoid wetting foliage.</li>
          <li className="flex gap-2"><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-forest-800 text-[11px] font-bold text-white">3</span>{expert ? "Wait for expert confirmation before any spray. Bio-protectant (neem 3 ml/L) is safe meanwhile." : "Start the IPM ladder in the advisory. Chemical control only if the threshold is crossed."}</li>
        </ol>
